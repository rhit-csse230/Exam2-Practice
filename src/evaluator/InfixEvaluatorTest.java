...
public class InfixEvaluatorTest {
	...
	public static void tearDown() throws Exception {
		// Uncomment the next lines if you'd like to try implementing this.
		// You won't be awarded any points, but you can use the printout to track
		// progress.
		String message = String.format("You earned %d/29 points for infix", points);
		System.out.println(message);
		System.out.println("Evaluator is optional.");
	}
	...
	public void testConvertToPostfixCheckPrecedence() {
		assertEquals("3 4 * 2 -", this.infixEvaluator.convertToPostfix("3 * 4 - 2"));
		assertEquals("7 6 5 * +", this.infixEvaluator.convertToPostfix("7 + 6 * 5"));
		points += 1;
	}
	...
	public void testConvertToPostfixMultipleOperators() {
		assertEquals("5 6 * 7 4 * +", this.infixEvaluator.convertToPostfix("5 * 6 + 7 * 4"));
		assertEquals("6 4 + 3 - 2 + 1 -", this.infixEvaluator.convertToPostfix("6 + 4 - 3 + 2 - 1"));
		points += 1;
	}
	...
	public void testConvertToPostfixPrecedenceWithExponents() {
		assertEquals("4 5 6 ^ * 2 +", this.infixEvaluator.convertToPostfix("4 * 5 ^ 6 + 2"));
		points += 1;
	}
	...
	public void testConvertToPostfixParentheses() {
		assertEquals("6 5 4 + *", this.infixEvaluator.convertToPostfix("6 * ( 5 + 4 )"));
		assertEquals("5 10 + 3 /", this.infixEvaluator.convertToPostfix("( 5 + 10 ) / 3"));
		points += 1;
	}
	...
	public void testConvertToPostfixParenthesesWithExponents() {
		assertEquals("6 5 4 3 * + 2 ^ *", this.infixEvaluator.convertToPostfix("6 * ( 5 + 4 * 3 ) ^ 2"));
		assertEquals("4 2 3 4 * + ^", this.infixEvaluator.convertToPostfix("4 ^ ( 2 + 3 * 4 )"));
		points += 1;
	}
	...
	public void testEvaluate() {
		...
		assertEquals(1734, this.infixEvaluator.evaluate("6 * ( 5 + 4 * 3 ) ^ 2"));
	...
	public void testDoubleParentheses() {
		assertEquals("8 6 + 7 2 - *", this.infixEvaluator.convertToPostfix("( 8 + 6 ) * ( 7 - 2 ) "));
		points += 1;
	}

	@Test
	public void testParenthesesToAPower() {
		assertEquals("3 3 + 3 3 + ^", this.infixEvaluator.convertToPostfix("( 3 + 3 ) ^ ( 3 + 3 )"));
		points += 1;
	}
...
//	 */
//	@Test
//	public void testEvaluateMultipleExponents() {
//		// FIXME: This test doesn't distinguish between alternatives of how to
//		// handle power to a power.
//		// since (2^2)^2 == 2^(2^2) = 16. Mine does first, but should be latter.
//		// Remove for now until clarify spec.
//		String infix = "2 ^ 2 ^ 2";
//		String postfix = this.infixEvaluator.convertToPostfix(infix);
//		// Asserting the postfix would clarify the issue.
//		assertEquals(16, this.infixEvaluator.evaluate(infix));
//		points += 1;
//	}
	...
	public void testEvaluateMultipleParentheses() {
		...
		assertEquals("2 3 7 * + 4 1 + /", this.infixEvaluator.convertToPostfix(infix));
	...
	public void testEvaluateLong1() {
		assertEquals(75, this.infixEvaluator
	...
	public void testEvaluateLong2() {
		assertEquals(36, this.infixEvaluator.evaluate("7 * 4 + 5 + 2 / 2 - 4 + 6"));
		points += 1;
	}

	@Test
	public void testEvaluateLong3() {
		assertEquals(852, this.infixEvaluator.evaluate("4 * 6 * 7 * 5 + 8 / 2 * 3"));
		points += 1;
	}
	...
	public void testEvaluateMultipleNestedLoopsAndTwoDigitNumbers1() {
		assertEquals(0, this.infixEvaluator.evaluate("( 3 + ( 3 + 3 ) * 6 ) - 39"));
		assertEquals(100, this.infixEvaluator.evaluate("( 10 + ( 20 - 30 ) * 2 ) ^ 2"));
		points += 2;
	}
	...
	public void testConvertToPostfixTest1() {
		assertEquals("45 5 + 10 /", this.infixEvaluator.convertToPostfix("( 45 + 5 ) / 10"));
		assertEquals(50, this.infixEvaluator.evaluate("( 45 + 5 )"));
		assertEquals("4 5 * 3 + 2 +", this.infixEvaluator.convertToPostfix("4 * 5 + 3 + 2"));
		points += 1;
	}
	...
	public void testNestedExponents() {
		...
		assertEquals(43046721, this.infixEvaluator.evaluate("3 ^ ( ( 2 ^ 2 ) ^ 2 )"));
		points += 1;
	}
	...
	public void testEvaluateParenGroups() {
		assertEquals(18, this.infixEvaluator.evaluate("( 3 * 2 + 7 ) + ( 10 / 2 )"));
		points += 1;
	}
